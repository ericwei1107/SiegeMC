...
- Arena snapshot capture is limited by `arena-reset.max-snapshot-blocks: 1000000`. Oversized selections are rejected before any snapshot files are touched.
...
4. Use LuckPerms to prove that a normal player lacks all staff nodes, especially `siege.admin`, `siege.admin.resetscores`, `siege.minecart.cooldown.bypass`, Towny administration, Multiverse administration, Vulcan administration, and LuckPerms administration.
...
2. Run a 20-minute session with 25 players: banner contention, shop and kit use, normal minecart placement/detonation, and one complete map reset.
...
5. Record the spark link, player count, active-cart peak, reset tile count/duration, and any configuration deviation beside the release artifact.

## Ongoing third-party review

Installed plugins currently include ClearLag, CombatLog, LuckPerms, Towny, TownyChat, Vulcan, Multiverse-Core, PacketEvents, and SiegePlugin. Treat each JAR as a separate supply-chain component: use vendor release notes and security advisories, update only after staging validation, and remove plugins that are unsupported or no longer needed.
